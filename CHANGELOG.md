...
- A cross-launch lifetime protocol for reusable extraction and FUSE targets. Every application tree holds a shared target lease, while cleanup requires the coordinator plus an exclusive lease before removing or unmounting the target. On OFD-capable kernels, coordinator, lifetime, and record-publication locks occupy three byte ranges of one protected adjacent inode, which remains as the stable lock identity after target cleanup.
- Application supervision through `PR_SET_CHILD_SUBREAPER` so no-proc cleanup can wait for daemonized and double-forked descendants even when they close every inherited file descriptor.
...
- Extracted-directory reuse remains hash-based and does not require procfs because no live helper or namespace transition must be authenticated. Concurrent launches now share one target lifetime lease, preventing the first launch from deleting the directory while another launch is still using it.
...
- Prevented the first reusable extraction launch from deleting a shared directory while a later launch or daemonized descendant still uses it.
...
- Normal no-proc execution and cleanup are supported without weakening private namespace authentication.
- On kernels or seccomp profiles where `PR_SET_CHILD_SUBREAPER` is unavailable, procfs remains the descendant-visibility fallback. If neither mechanism is available, uruntime retains the target instead of risking cleanup beneath an unobservable daemon.
